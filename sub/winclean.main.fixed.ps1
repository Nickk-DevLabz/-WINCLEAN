# ==============================================================================
# WINCLEAN PRO v5.0 - FULL TECHNICIAN SUITE (FIXED)
...
$btnRunCleanup.Add_Click({
    ...
    # Optional: Enhanced registry unlock for system apps
    if ($chkUnlock.Checked) {
        try {
            # Remove IsInbox flags to allow removal
            ...
            # Also remove System app flag
            $stateRoot = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Appx\AppxAllUserStore\State"
            if (Test-Path $stateRoot) {
                Get-ChildItem $stateRoot -ErrorAction SilentlyContinue | ForEach-Object {
                    Set-ItemProperty -Path $_.PSPath -Name "IsInbox" -Value 0 -ErrorAction SilentlyContinue
                }
            }
        } catch {
            # Silently continue if registry unlock fails
        }
    }
    ...
    foreach ($item in $selected) {
        ...
        try {
            # Remove installed packages (current user context)
            $packages = Get-AppxPackage -AllUsers -ErrorAction SilentlyContinue | Where-Object { $_.Name -like "*$item*" }
            
            if ($packages) {
                $appFound = $true
                foreach ($pkg in $packages) {
                    try {
                        # FIXED: Removed -AllUsers parameter from Remove-AppxPackage as it causes syntax conflicts with -Package
            ...
            # Remove provisioned packages (pre-installed system apps for all users)
            $provisionedPkgs = Get-AppxProvisionedPackage -Online -ErrorAction SilentlyContinue | 
                Where-Object { $_.DisplayName -like "*$item*" -or $_.PackageName -like "*$item*" }
                ...
                foreach ($pkg in $provisionedPkgs) {
                    try {
                        ...
                        if ($_.Exception.Message -match "0x80070005") {
                            $errorMsg = "Access Denied - Try: 1) Unlock System Apps, 2) Close related windows, 3) Run Deep Clean in System Tweaks"
                        } else {
                            $errorMsg = $_.Exception.Message
                        }
                    }
            ...
            if ($appFound) {
                if ($errorMsg) {
                    $results["Failed"] += "$item`n    Error: $errorMsg"
                } else {
                    $results["Success"] += $item
                }
            } else {
                $results["NotFound"] += $item
            }

        } catch {
            $results["Failed"] += "$item`n    Error: $($_.Exception.Message)"
        }
    }
    ...
    if ($chkShell.Checked) {
        try {
            ...
            # Continue if shell repair fails
        }
    }

    # Build detailed report
    $report = "=== APP REMOVAL REPORT ===" + [Environment]::NewLine + [Environment]::NewLine
    
    if ($results["Success"].Count -gt 0) {
        $report += "[SUCCESS] REMOVED ($($results["Success"].Count)):" + [Environment]::NewLine
        foreach ($app in $results["Success"]) { $report += "  - $app" + [Environment]::NewLine }
        $report += [Environment]::NewLine
    }
    
    if ($results["Failed"].Count -gt 0) {
        $report += "[FAILED] REMOVAL FAILED ($($results["Failed"].Count)):" + [Environment]::NewLine
        foreach ($app in $results["Failed"]) { $report += "  - $app" + [Environment]::NewLine }
        $report += [Environment]::NewLine
    }
    
    if ($results["NotFound"].Count -gt 0) {
        $report += "[NOT FOUND] NOT INSTALLED ($($results["NotFound"].Count)):" + [Environment]::NewLine
        foreach ($app in $results["NotFound"]) { $report += "  - $app" + [Environment]::NewLine }
        $report += [Environment]::NewLine
    }

    $report += [Environment]::NewLine + "NOTES:" + [Environment]::NewLine
    $report += "- Access Denied errors: Enable 'Unlock Permanent System Apps' in Tab 1" + [Environment]::NewLine
    $report += "- System apps may need Deep Clean (Tab 3) to fully remove" + [Environment]::NewLine
    $report += "- Some apps may require a reboot to take effect"

    Reload-AppList
    $btnRunCleanup.Enabled = $true
    $btnRunCleanup.Text    = "EXECUTE SELECTED APP REMOVAL"
    [System.Windows.Forms.MessageBox]::Show($report, "Removal Results", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
})
...
$btnInstall.Add_Click({
    ...
    foreach ($name in $selected) {
        ...
        if ($entry) {
            # Capture process exit code for result reporting
            $proc = Start-Process winget -ArgumentList "install --id $($entry.Id) --silent --accept-package-agreements --accept-source-agreements" -Wait -PassThru -NoNewWindow
    ...
    $report = "Installation complete." + [Environment]::NewLine
    if ($results["Success"].Count -gt 0) { $report += "[✓] Success: $($results["Success"] -join ', ')" + [Environment]::NewLine }
    if ($results["Failed"].Count -gt 0) { $report += "[X] Failed: $($results["Failed"] -join ', ')" }
    
    [System.Windows.Forms.MessageBox]::Show($report, "Done")
})
...
$btnDiag.Add_Click({
    ...
    # FIXED: Explicitly import Storage module for physical disk info
...
try {
    Write-Host "[✓] Form initialized with $($TabCtrl.TabPages.Count) tabs" -ForegroundColor Green
    Write-Host ""
    Write-Host "Launching GUI window..." -ForegroundColor Yellow
    Write-Host "Close the GUI window to exit" -ForegroundColor Gray
    Write-Host ""
    
    # FIXED: Corrected Launch Initialization. Populating list and showing dialog.
    Reload-AppList
    [void]$Form.ShowDialog()
    
    Write-Host "`n`nForm closed - session ended." -ForegroundColor Green
    Write-Host "You can close this window or press Ctrl+C" -ForegroundColor Gray
    ...
    Write-Host "`nERROR at line $errorLine : $errorMsg" -ForegroundColor Red
    Write-Host "Stack Trace: $($_.ScriptStackTrace)" -ForegroundColor Red
    
    [System.Windows.Forms.MessageBox]::Show(
        "Fatal error at line $errorLine`:`n`n$errorMsg`n`nStack: $($_.ScriptStackTrace)",
        "WinClean Error",
        [System.Windows.Forms.MessageBoxButtons]::OK,
        [System.Windows.Forms.MessageBoxIcon]::Error
    )
    
    Write-Host "`n`nKeeping window open for 10 seconds to show error..." -ForegroundColor Red
    Start-Sleep -Seconds 10
}

Write-Host "WinClean Pro v5.0 - Session ended." -ForegroundColor Cyan